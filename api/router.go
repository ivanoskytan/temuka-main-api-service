...
func Routes(db database.PostgresWrapper, redis keyValueStore.RedisWrapper, storage fileStorage.AzureBlobWrapper, rmq queue.RabbitMQChannel, hub *websocket.Hub) *mux.Router {
	...
	postRouter.HandleFunc("/like/{id}", postHandler.LikePost).Methods("POST")
	postRouter.HandleFunc("/unlike/{id}", postHandler.UnlikePost).Methods("POST")
	postRouter.HandleFunc("/save/{id}", postHandler.SavePost).Methods("POST")
	postRouter.HandleFunc("/unsave/{id}", postHandler.UnsavePost).Methods("POST")
